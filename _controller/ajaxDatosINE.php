...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['image'])) {
    ...
    if (is_uploaded_file($tempFile)) {
      ...
      $response['fulltext_spa'] = file_get_contents($outputFile . '.txt');
      exec("tesseract $tempFile $outputFile -l eng", $output, $returnCode);
      // Depuración: Verifica si Tesseract se ejecutó
      $response['exec_output'] = $output;
      $response['exec_return_code'] = $returnCode;
      ...
      $response['status'] = 'success';
      $response['fulltext_eng'] = file_get_contents($outputFile . '.txt');
      ...
      //Validamos el nombre
      if (preg_match($patternName, $response['fulltext_spa'], $matches) || preg_match($patternName, $response['fulltext_eng'], $matches)) {
        $splitName = [];
        $name = trim($matches[0], "\n");
        $cont = 0;
        $arrayName = explode("\n", $name);
      ...
      if (preg_match($patternAddress, $response['fulltext_spa'], $matches) || preg_match($patternAddress, $response['fulltext_eng'], $matches)) {
      ...
      //Validamos el CURP
      if (preg_match($patternCURP, $response['fulltext_spa'], $matches) || preg_match($patternCURP, $response['fulltext_eng'], $matches)) {
